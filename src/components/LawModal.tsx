...
/**
 * Доступная модалка с текстом раздела «Правовая информация» (см. src/content/law.ts).
 * Структура повторяет PrivacyPolicyModal/TermsModal. a11y: role="dialog" + aria-modal,
 * Escape закрывает, клик по подложке закрывает, фокус уходит на кнопку закрытия при
 * открытии и возвращается на вызвавший элемент при закрытии, body overflow заблокирован
 * пока открыта, фокус-трап через Tab.
 */
export function LawModal({ open, onClose }: LawModalProps) {
  const reduceMotion = useReducedMotion()
  const dialogRef = useRef<HTMLDivElement>(null)
  const closeButtonRef = useRef<HTMLButtonElement>(null)
  const previouslyFocused = useRef<HTMLElement | null>(null)
  const titleId = 'law-modal-title'
          ...
          >
            <div className="flex items-start justify-between gap-4 border-b border-sky-100 px-6 py-5 md:px-8">
              <div>
                ...
                <p className="mt-1 text-xs text-ink-soft">Обновлено: {law.updatedAt}</p>
              </div>
              <button
                ref={closeButtonRef}
                type="button"
            ...
            <div className="overflow-y-auto px-6 py-6 md:px-8">
              <p className="text-sm text-ink-soft">{law.intro}</p>
              ...
              <div className="mt-6 border-t border-sky-100 pt-5">
                <h3 className="text-base font-semibold text-ink">Контакты</h3>
                <div className="mt-2 flex flex-col gap-1 text-sm leading-relaxed text-ink-soft">
                  <p>
                    Email:{' '}
                    <a href={`mailto:${law.contacts.email}`} className="text-sky-600 hover:underline">
                      {law.contacts.email}
                    </a>
                  </p>
                  <p>
                    Telegram-бот:{' '}
                    <a
                      href={law.contacts.telegramBot}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-sky-600 hover:underline"
                    >
                      {law.contacts.telegramBot}
                    </a>
                  </p>
                  <p>
                    Telegram-канал:{' '}
                    <a
                      href={law.contacts.telegramChannel}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-sky-600 hover:underline"
                    >
                      {law.contacts.telegramChannel}
                    </a>
                  </p>
                </div>
              </div>
