...
/**
 * Доступная модалка с текстом пользовательского соглашения (см. src/content/terms.ts).
 * Структура повторяет PrivacyPolicyModal. a11y: role="dialog" + aria-modal, Escape
 * закрывает, клик по подложке закрывает, фокус уходит на кнопку закрытия при открытии
 * и возвращается на вызвавший элемент при закрытии, body overflow заблокирован пока
 * открыта, фокус-трап через Tab.
 */
export function TermsModal({ open, onClose }: TermsModalProps) {
  ...
  const titleId = 'terms-modal-title'
          ...
          >
            <div className="flex items-start justify-between gap-4 border-b border-sky-100 px-6 py-5 md:px-8">
              <div>
                ...
                <p className="mt-1 text-xs text-ink-soft">Обновлено: {terms.updatedAt}</p>
              </div>
              <button
                ref={closeButtonRef}
                type="button"
            ...
            <div className="overflow-y-auto px-6 py-6 md:px-8">
              <p className="text-sm text-ink-soft">{terms.intro}</p>
